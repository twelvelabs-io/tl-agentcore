...
variable "agent_model_id" {
  ...
  description = "Bedrock cross-region inference profile for the Strands orchestrator. Sonnet 4.6 mirrors Jockey's frontier-model class. With kb_cache pre-built, the agent finishes in fewer turns so the per-turn latency hit is offset."
}
