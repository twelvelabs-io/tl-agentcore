...
Operator-facing notes live in the companion doc
[`deployment.md`](deployment.md): what the stack provisions, the
`terraform apply` + container-push + vector-ingest sequence, and the
implementation notes worth knowing before the first apply (arm64-only
runtime, WebSocket 30 s cap, async-retry phantom duplicates, S3 Vectors
metadata caps, and the rest).
